...
- **`react` must match RN 0.81.5's peer dependency (`19.1.0`) exactly, same principle as the worklets pin above.** `my-new-app/package.json` correctly declares `"react": "19.1.0"` (confirmed via `npx expo install --check`), but some other dependency in the tree pulls a newer `react` (seen: `19.2.7`) that npm hoists to the _root_ `node_modules`, leaving two live React copies — one at root, one nested under `my-new-app/node_modules` since the declared version didn't match what got hoisted. Metro bundles both, and the app crashes immediately on launch with "Invalid hook call" / "Cannot read property 'useEffect' of null" from `app/index.tsx`. Fixed by adding `"react": "19.1.0"` to the root `package.json`'s `"overrides"` block and doing a full clean reinstall (see the corrupted-node_modules landmine below for the kill-processes-first procedure). If this recurs after a dependency bump, verify with `npm ls react` from `my-new-app/` — it should show a single deduped version, not two.
